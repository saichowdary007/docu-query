        ...
        if output == 'json':
            ...
            if not documents:
                ...
                for doc in documents:
                    click.echo(f"   • {doc['filename']} ({doc['file_type']}) - ID: {doc['file_id']}")
                    if doc['created_at']:
...
def init(ctx):
    ...
    if not google_api_key:
        click.echo("⚠️  GOOGLE_API_KEY not found. Please set it as an environment variable.")
        click.echo("   export GOOGLE_API_KEY='your-api-key'")
    
    if not google_project_id:
        click.echo("⚠️  GOOGLE_PROJECT_ID not found. Please set it as an environment variable.")
